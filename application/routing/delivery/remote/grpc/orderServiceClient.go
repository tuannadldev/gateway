...
func (client *orderServiceClient) createRequest(data interface{}, md map[string]string) (interface{}, error) {
    ctx := monitor.GetApmContext()
    //ctx := context.Background()
	if len(md) > 0{
        ctx = metadata.NewOutgoingContext(
    ...
    return client.grpcClient.CreateRequest(ctx, data.(*order.GetListRequest))
}
